...
router.post("/api/:conn/:db/:coll/:page", function (req, res, next) {
  ...
  var docs_per_page =
    parseInt(req.body.docsPerPage) !== undefined
      ? parseInt(req.body.docsPerPage)
      : 5;
  ...
  mongo_db
    ...
    .find(query_obj, { skip: skip, limit: limit })
      ...
      if (err) {
        ...
        mongo_db
          ...
          .toArray(function (err, simpleSearchFields) {
            ...
            mongo_db
              .collection(req.params.coll)
              .count(query_obj, function (err, doc_count) {
                var return_data = {
                  data: result,
                  fields: fields,
  ...
  req.db
    ...
    .exec(function (err, serverEvents) {
      ...
      if (serverEvents.length > 0) {
          ...
          if (serverEvents) {
            _.each(serverEvents, function (value, key) {
              ...
              if (value.connections) {
                connectionsCurrent.push({
                  x: value.eventDate,
                  y: value.connections.current,
                });
                connectionsAvailable.push({
                  x: value.eventDate,
                  y: value.connections.available,
                });
              ...
              if (value.activeClients) {
                clientsTotal.push({
                  x: value.eventDate,
                  y: value.activeClients.total,
                });
                clientsReaders.push({
                  x: value.eventDate,
                  y: value.activeClients.readers,
                });
                clientsWriters.push({
                  x: value.eventDate,
                  y: value.activeClients.writers,
                });
              }
              // memory
              if (value.memory) {
                memoryVirtual.push({
                  x: value.eventDate,
                  y: value.memory.virtual,
                });
                memoryMapped.push({
                  x: value.eventDate,
                  y: value.memory.mapped,
                });
                memoryCurrent.push({
                  x: value.eventDate,
                  y: value.memory.resident,
                });
              }
              ...
              if (value.docCounts) {
                docsQueried.push({
                  x: value.eventDate,
                  y: value.docCounts.queried,
                });
                docsInserted.push({
                  x: value.eventDate,
                  y: value.docCounts.inserted,
                });
                docsDeleted.push({
                  x: value.eventDate,
                  y: value.docCounts.deleted,
                });
                docsUpdated.push({
                  x: value.eventDate,
                  y: value.docCounts.updated,
                });
              }
            });
          }
        ...
        var returnedData = {
          connectionsCurrent: averageDatapoints(
            connectionsCurrent,
            dataPointsLimit
          ),
          connectionsAvailable: averageDatapoints(
            connectionsAvailable,
            dataPointsLimit
          ),
          connectionsTotalCreated: averageDatapoints(
        ...
        if (err) {
          res
            .status(400)
            .json({ msg: req.i18n.__("Could not get server monitoring") });
        } else {
          res.status(200).json({
            data: returnedData,
