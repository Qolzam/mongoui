...
// runs on all routes and checks p assword if one is setup
router.all("/*", common.checkLogin, function (req, res, next) {
  if (
    req.nconf.app.get("app") &&
    req.nconf.app.get("app").hasOwnProperty("password")
  ) {
...
router.get("/app/monitoring/:conn/", function (req, res, next) {
  ...
  if (req.nconf.app.get("app:monitoring") === false) {
    ...
    monitoringMessage =
      "Monitoring has been switched off in the config. Please enable or remove if you want this feature.";
  }
...
router.get("/app/:conn", function (req, res, next) {
  ...
  if (connection_list[req.params.conn] === undefined) {
    common.render_error(
      res,
      req,
      req.i18n.__("Invalid connection name"),
      req.params.conn
    );
    return;
  }
  ...
  if (uri.database) {
    res.redirect(
      req.app_context + "/app/" + req.params.conn + "/" + uri.database
    );
    return;
  }
  ...
  common.get_db_status(mongo_db, function (err, db_status) {
    common.get_backups(function (err, backup_list) {
      common.get_db_stats(mongo_db, uri.database, function (err, db_stats) {
        common.get_sidebar_list(
          mongo_db,
          uri.database,
          function (err, sidebar_list) {
...
router.get("/app/:conn/:db", function (req, res, next) {
  ...
  if (req.params.db.indexOf(" ") > -1) {
    common.render_error(
      res,
      req,
      req.i18n.__("Invalid database name"),
      req.params.conn
    );
    return;
  }
  ...
  // do DB stuff
  common.get_db_stats(mongoClient, req.params.db, function (err, db_stats) {
    common.get_sidebar_list(
      mongoClient,
      req.params.db,
      function (err, sidebar_list) {
          ...
          .count(function (err, coll_count) {
            if (collection_list.indexOf(req.params.coll) === -1) {
              ...
              res.render("coll-view", {
                conn_list: common.order_object(
                  req.nconf.connections.get("connections")
                ),
                conn_name: req.params.conn,
                db_name: req.params.db,
        ...
        function (err, result) {
          if (result.doc === undefined) {
            ...
            common.render_error(
              res,
              req,
              req.i18n.__("Document not found"),
              req.params.conn
            );
            return;
          }
            ...
            if (value) {
              if (value.toString().substring(0, 10) === "data:video") {
                videos.push({
                  field: key,
                  src: value,
                  type: value.split(";")[0].replace("data:", ""),
                });
              }
            }
          });
